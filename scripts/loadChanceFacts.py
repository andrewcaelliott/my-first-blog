...
def addChanceFact(title="Test Fact", text="Just a trial", probability = "1 in 100", item_text = "items", exposed_items="50", repetition_text = "repetitions", exposed_repetitions = 20, repeat_mode = "repeats", outcome_text="hits"):
...
def loadChanceFacts(fileName, prefix, encoding="UTF-8"):
    ...
    for line in lines[1:]:
        fields = line[:-1].split(",")
        ordinal, subject, probability, items, item_count, repetitions, repetition_count, hits, repeats, source = fields
        print(ordinal, subject, probability, items, item_count, repetitions, repetition_count, hits, repeats, source)
        fact = addChanceFact(title=subject, text=source, probability = probability, item_text = items, exposed_items=item_count, repetition_text = repetitions, exposed_repetitions = repetition_count, repeat_mode=repeats, outcome_text=hits)
